...
func Test_GenerateEnterpriseCode(t *testing.T) {
	t.Logf("Given the need to generate code for enterprise")
	{
		code := validate.GenerateEnterpriseCode()
		t.Logf("\tTest %v:\tcheck generated code", code)
		{
			if len(code) != 6 {
				t.Fatalf("\tTest %v:\tcode should be at least of length 6", code)
			}
			if _, err := strconv.Atoi(code); err != nil {
				t.Fatalf("\tTest %v:\tcode should be a valid integer value %v", code, err)
			}
		}
	}
}
